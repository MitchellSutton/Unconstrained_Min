''' Polak-Ribiere Conjugate Gradient Method for Approximating Minimizers to 2D Functions '''

#The Polak-Ribiere conjugate gradient method for approximating minimizers to a given function relys on the following:
...
#       Compute search direction parameter (unique to Polak-Ribiere)
...
def POLAK_RIBIERE(U):
    ...
    #iterate method until tolerance is achienved
    while error > 10e-5:
        #calculate and store gradient at previous step
        DF = np.append(DF,[gradf(U[k])],axis=0) 
        #calculate search direction parameter according to Polak-Ribiere
        b = (np.dot(gradf(U[k]), gradf(U[k])-gradf(U[k-1])))/(la.norm(gradf(U[k-1]))**2)
        #update and store search direction according to Polak-Ribiere
